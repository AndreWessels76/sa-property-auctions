/**
 * Historical Intelligence 5.4 — production live validation (read-only).
 * Run: npm run hi54:live
 */
const fs = require("fs");
...
function writeArtifacts(report) {
  const livePath = path.join(root, "HISTORICAL_INTELLIGENCE54_LIVE.json");
  const evidencePath = path.join(root, "HISTORICAL_INTELLIGENCE54_EVIDENCE.json");
  const reportPath = path.join(root, "HISTORICAL_INTELLIGENCE54_REPORT.md");
  const gapPath = path.join(root, "HISTORICAL_INTELLIGENCE54_GAP_REPORT.md");
    ...
    JSON.stringify(
      {
        ...
        evidenceFunnel54: report.evidenceFunnel54,
        bottleneck53: report.bottleneck53,
        bottleneck54: report.bottleneck54,
        bottleneckRanked53: report.bottleneckRanked53,
        bottleneckRanked54: report.bottleneckRanked54,
        coverageRates: report.coverageRates,
        evidenceQualityCounts: report.evidenceQualityCounts,
  ...
  const md = `# Historical Intelligence 5.4 — Live Report
...
**${report.campaign54?.status ?? report.campaign?.status ?? "—"}**

${report.p1Progress54 ? `P1 Progress [${report.p1Progress54.progressBar}] ${report.p1Progress54.progressLabel}\nRemaining: ${report.p1Progress54.remaining}` : report.campaign?.progressBar ?? ""}

${report.campaign54?.summaryLine ?? report.campaign?.summaryLine ?? ""}
...
${(report.evidenceFunnel54 ?? report.evidenceFunnel ?? []).map((s, i) => `${i === 0 ? "" : "↓ "}${s.value} ${s.label}`).join("\n")}

## BOTTLENECK

**${report.bottleneck54?.code ?? report.bottleneck53?.code ?? report.bottleneck?.code ?? "—"}** — ${report.bottleneck54?.count ?? report.bottleneck53?.count ?? report.bottleneck?.count}/${report.bottleneck54?.total ?? report.bottleneck53?.total ?? report.bottleneck?.total} (${report.bottleneck54?.percentage ?? "—"}%)

Recommended: ${report.bottleneck54?.recommendedAction ?? report.bottleneck53?.recommendedAction ?? report.bottleneck?.recommendedAction}

${(report.bottleneckRanked54 ?? report.bottleneckRanked53 ?? []).map((b) => `- ${b.code}: ${b.count}/${b.total} → ${b.recommendedAction}`).join("\n")}

## PUBLIC SAFETY

Catalogue leaks: **${report.safety?.catalogueLeaks ?? report.coverage52?.catalogueLeaks ?? 0}**
Rebuild status: **${report.safety?.rebuildStatus ?? "—"}**
...
async function main() {
  ...
  if (envPresent) {
    ...
    if (
      ...
      fs.writeFileSync(
        path.join(root, "HISTORICAL_INTELLIGENCE54_LIVE.json"),
