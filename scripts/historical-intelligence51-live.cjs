/**
 * Historical Intelligence 5.1 — production live validation (read-only).
 * Run: npm run hi51:live
 */
const fs = require("fs");
...
async function buildProductionReport(db) {
  ...
  return buildHi51Report({ hscReport, enrichmentRuns: enrichmentRuns ?? [] });
}

function writeArtifacts(report) {
  const livePath = path.join(root, "HISTORICAL_INTELLIGENCE51_LIVE.json");
  const evidencePath = path.join(root, "HISTORICAL_INTELLIGENCE51_EVIDENCE.json");
  const reportPath = path.join(root, "HISTORICAL_INTELLIGENCE51_REPORT.md");
  const gapPath = path.join(root, "HISTORICAL_INTELLIGENCE51_GAP_REPORT.md");
    ...
    JSON.stringify(
      {
        ...
        investorLabels: report.investorLabels,
  ...
  const md = `# Historical Intelligence 5.1 — Live Report
...
| SOLD Without Price | ${report.coverageDashboard.soldWithoutPrice} |
| Verified Sale Prices | ${report.coverageDashboard.verifiedSalePrices} |
| Comparable Ready | ${report.coverageDashboard.comparableReady} |
| Market Ready Towns | ${report.coverageDashboard.marketReadyTowns} |
| Catalogue Leaks | ${report.coverageDashboard.catalogueLeaks} |

## P1 PROGRESS

Original: ${report.p1Progress?.originalCandidates ?? "—"} — Processed: ${report.p1Progress?.processed ?? "—"} — Remaining: ${report.p1Progress?.remaining ?? "—"}

## FETCH RESULTS

- Attempted: ${report.fetchResults?.attempted ?? "—"}
- Successful: ${report.fetchResults?.successful ?? "—"}
- Failed: ${report.fetchResults?.failed ?? "—"}
- Retryable: ${report.fetchResults?.retryable ?? "—"}
- Permanent: ${report.fetchResults?.permanent ?? "—"}
- Legacy: ${report.fetchResults?.legacy ?? "—"}
...
- Fetch: ${report.chainSuccessRates?.fetchSuccessRate ?? report.successRates?.fetchSuccessRate}
- Snapshot: ${report.chainSuccessRates?.snapshotRate ?? report.successRates?.snapshotRate}
- Extraction: ${report.chainSuccessRates?.extractionRate ?? "INSUFFICIENT_DATA"}
- Outcome evidence: ${report.chainSuccessRates?.outcomeEvidenceRate ?? report.successRates?.outcomeEvidenceRate}
- Sale price: ${report.chainSuccessRates?.salePriceRate ?? report.successRates?.verifiedSalePriceRate}
...
Controlled **Dry Run P1 (5)** then **Acquire P1 (5)** via Operations Centre — admin-initiated batches only.
`;
...
async function main() {
  ...
  if (envPresent) {
    ...
    if (
      ...
      fs.writeFileSync(
        path.join(root, "HISTORICAL_INTELLIGENCE51_LIVE.json"),
