/**
 * Historical Intelligence 5.2 — production live validation (read-only).
 * Run: npm run hi52:live
 */
const fs = require("fs");
...
async function buildProductionReport(db) {
  ...
  return buildHi52Report(buildHi51Report({ hscReport, enrichmentRuns: enrichmentRuns ?? [] }));
}

function writeArtifacts(report) {
  const livePath = path.join(root, "HISTORICAL_INTELLIGENCE52_LIVE.json");
  const evidencePath = path.join(root, "HISTORICAL_INTELLIGENCE52_EVIDENCE.json");
  const reportPath = path.join(root, "HISTORICAL_INTELLIGENCE52_REPORT.md");
  const gapPath = path.join(root, "HISTORICAL_INTELLIGENCE52_GAP_REPORT.md");
    ...
    JSON.stringify(
      {
        ...
        bottleneckRanked: report.bottleneckRanked,
        stateMachineCounts: report.stateMachineCounts,
  ...
  const md = `# Historical Intelligence 5.2 — Live Report
...
${(report.evidenceLabels?.provenInProduction ?? []).map((l) => `- ${l}`).join("\n") || "- (none)"}

## TESTED

${(report.evidenceLabels?.tested ?? []).map((l) => `- ${l}`).join("\n") || "- (none)"}

## ENGINE READY

${(report.evidenceLabels?.engineReady ?? []).map((l) => `- ${l}`).join("\n") || "- (none)"}

## INSUFFICIENT DATA

${(report.evidenceLabels?.insufficientData ?? []).map((l) => `- ${l}`).join("\n") || "- (none)"}

## REVIEW REQUIRED

${(report.evidenceLabels?.reviewRequired ?? []).map((l) => `- ${l}`).join("\n") || "- (none)"}

## PRODUCTION COUNTS

| Metric | Value |
|--------|-------|
| Historical Events | ${report.coverage52?.historicalEvents ?? report.coverageDashboard.historicalEvents} |
| Licensed Sources | ${report.coverage52?.licensedSources ?? report.coverageDashboard.licensedSources} |
| Fetch Attempted | ${report.coverage52?.fetchAttempted ?? report.coverageDashboard.fetchAttempted} |
| Never Attempted | ${report.coverage52?.neverAttempted ?? report.coverageDashboard.neverAttempted} |
| Fetch Successful | ${report.coverage52?.fetchSuccessful ?? report.coverageDashboard.fetchSuccessful} |
| Fetch Failed | ${report.coverage52?.fetchFailed ?? report.coverageDashboard.fetchFailed} |
| Retryable | ${report.coverage52?.retryable ?? "—"} |
| Permanent | ${report.coverage52?.permanent ?? "—"} |
| Legacy Failures | ${report.coverage52?.legacyFailures ?? report.coverageDashboard.legacyFailuresRequiringRefetch} |
| Snapshots | ${report.coverage52?.snapshots ?? report.coverageDashboard.snapshots} |
| Missing Extraction | ${report.coverage52?.missingExtraction ?? "—"} |
| Extractions | ${report.coverage52?.extractions ?? report.coverageDashboard.extractions} |
| Outcome Evidence | ${report.coverage52?.outcomeEvidence ?? report.coverageDashboard.outcomeEvidence} |
| Verified SOLD | ${report.coverage52?.verifiedSold ?? report.coverageDashboard.verifiedSold} |
| SOLD Without Price | ${report.coverage52?.soldWithoutPrice ?? report.coverageDashboard.soldWithoutPrice} |
| Verified Sale Prices | ${report.coverage52?.verifiedSalePrices ?? report.coverageDashboard.verifiedSalePrices} |
| Comparable Ready | ${report.coverage52?.comparableReady ?? report.coverageDashboard.comparableReady} |
| Market Ready Towns | ${report.coverage52?.marketReadyTowns ?? report.coverageDashboard.marketReadyTowns} |
| Catalogue Leaks | ${report.coverage52?.catalogueLeaks ?? report.coverageDashboard.catalogueLeaks} |

## BOTTLENECK

**${report.bottleneck.code ?? report.bottleneck.primary}** — ${report.bottleneck.count}/${report.bottleneck.total}

Recommended: ${report.bottleneck.recommendedAction}

${(report.bottleneckRanked ?? []).map((b) => `- ${b.code}: ${b.count}/${b.total} → ${b.recommendedAction}`).join("\n")}

## STAGES

${(report.stages ?? []).map((s) => `- **${s.id}** ${s.label}: eligible ${s.eligible}, next ${s.nextBatch}, remaining ${s.remaining}`).join("\n")}
...
${report.nextAdminAction ?? "Dry Run P1 (5) then Acquire P1 (5)"}
`;
...
async function main() {
  ...
  if (envPresent) {
    ...
    if (
      ...
      fs.writeFileSync(
        path.join(root, "HISTORICAL_INTELLIGENCE52_LIVE.json"),
